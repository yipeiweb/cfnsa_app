...
export default function LeagueStatsTab() {
  ...
  // 🔗 1. 初始化状态：从 URL 统一解析所有检索参数 (新增门将子 Tab 映射)
  const [subTab, setSubTab] = useState(() => {
    const params = new URLSearchParams(window.location.hash.split("?")[1]);
    return params.get("statsTab") || "total"; // 可选: total | round | goalkeeper_total | goalkeeper_round
  });

  // 拆分进球/助攻榜 或 扑救/零封榜 (默认 'goals'，门将模式下可选 'saves' | 'clean_sheets')
  const [rankType, setRankType] = useState(() => {
    const params = new URLSearchParams(window.location.hash.split("?")[1]);
    const urlRank = params.get("rankType");
    if (urlRank) return urlRank;
    // 如果是门将模式，默认排扑救
    const currentTab = params.get("statsTab") || "total";
    return currentTab.startsWith("goalkeeper") ? "saves" : "goals";
  });
  ...
  // 🚨 熔断硬锁：如果当前选中的是中女超且不在门将模式，锁死进球榜
  useEffect(() => {
    if (
      searchLeague === "中女超" &&
      !subTab.startsWith("goalkeeper") &&
      rankType !== "goals"
    ) {
      setRankType("goals");
    }
  }, [searchLeague, rankType, subTab]);
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      const urlRankType =
        params.get("rankType") ||
        (urlTab.startsWith("goalkeeper") ? "saves" : "goals");
  ...
  useEffect(() => {
    ...
    if (subTab === "round" || subTab === "goalkeeper_round") {
  ...
  useEffect(() => {
    async function fetchLeagueStats() {
      try {
        const { data } = await supabase.from("match_stats").select(`
            *,
  ...
  const availableSeasons = useMemo(() => {
    const seasons = statsData
      .map((item) => item.season?.toString())
      .filter(Boolean);
    return [...new Set(seasons)].sort((a, b) => b - a);
  }, [statsData]);

  const availableBirthYears = useMemo(() => {
    const years = statsData
      .map((item) => item.players?.birth_year)
      .filter(Boolean);
  ...
  const availableRounds = useMemo(() => {
    const filteredRounds = statsData
      .filter((item) => {
        ...
        const matchLeague =
          searchLeague === "all" || item.league === searchLeague;
    ...
    const maxRound =
      filteredRounds.length > 0 ? Math.max(...filteredRounds) : 30;
  ...
  const isPlayerMatchConditions = (item) => {
    ...
    const matchSeason =
      searchSeason === "all" || item.season?.toString() === searchSeason;

    const targetSeasonYear = parseInt(searchSeason, 10) || 2026;
    const u19Line = targetSeasonYear - 19;
    ...
    const matchBirthFrom =
      birthFrom === "all" ||
      (birthYear && birthYear >= parseInt(birthFrom, 10));
    const matchBirthTo =
      birthTo === "all" || (birthYear && birthYear <= parseInt(birthTo, 10));
  ...
  const aggregatedTotalData = useMemo(() => {
    ...
    filtered.forEach((item) => {
      ...
      playerMap[pid].saves += item.saves || 0;
      playerMap[pid].clean_sheets += item.clean_sheet ? 1 : 0;
    ...
    const finalFilteredArray = allPlayersArray.filter((player) => {
      if (isGkMode) {
        if (rankType === "clean_sheets") {
          return player.clean_sheets > 0; // 🛡️ 零封榜：只录取零封场次 > 0 的门将
        }
        return player.saves >= 1; // 🧤 扑救榜：录取扑救次数 >= 1 的门将
      } else {
  ...
  // 2. 🟢 每轮级数据处理 (同时支持常规/门将过滤)
  const roundData = useMemo(() => {
    const isGkMode = subTab.startsWith("goalkeeper");
    return statsData
      .filter((item) => {
        const matchRound = item.round?.toString() === searchRound;
        if (!matchRound || !isPlayerMatchConditions(item)) return false;

        if (isGkMode) {
          return item.saves >= 1 || item.clean_sheet === true;
        } else {
      ...
      .sort((a, b) => {
        if (isGkMode) {
          if (rankType === "saves") {
            if (b.saves !== a.saves) return b.saves - a.saves;
            const bCs = b.clean_sheet ? 1 : 0;
            const aCs = a.clean_sheet ? 1 : 0;
  ...
  const currentPlayerCount = useMemo(() => {
    return subTab === "total" || subTab === "goalkeeper_total"
      ? aggregatedTotalData.length
      : roundData.length;
  ...
  const exportStatsImageAndCopy = async () => {
    ...
    try {
      ...
      let typeText = "数据榜";
      if (subTab === "total")
        typeText = rankType === "goals" ? "赛季进球榜" : "赛季助攻榜";
      if (subTab === "round") typeText = `第${searchRound}轮进球助攻榜`;
      if (subTab === "goalkeeper_total")
        typeText = rankType === "saves" ? "赛季扑救榜" : "赛季零封榜";
      if (subTab === "goalkeeper_round")
        typeText = `第${searchRound}轮扑救零封榜`;
  ...
  const dynamicLeagueTitleText = useMemo(() => {
    if (searchLeague === "all")
      return "三级职业联赛（中超u23/中甲u21/中乙u19/中女超u23）";
  ...
  return (
    <div className="space-y-6">
      {/* 子导航 Sub-Tabs */}
      <div className="flex flex-wrap border-b border-gray-800 pb-px gap-1 md:gap-2">
        <button
          onClick={() => {
            setSubTab("total");
            setRankType("goals");
          }}
          className={`px-3 py-2 text-xs font-black tracking-wide border-b-2 transition-all ${
            subTab === "total"
        ...
        <button
          onClick={() => {
            setSubTab("goalkeeper_total");
            setRankType("saves");
          }}
          className={`px-3 py-2 text-xs font-black tracking-wide border-b-2 transition-all ${
            subTab === "goalkeeper_total"
        ...
        <button
          onClick={() => {
            setSubTab("goalkeeper_round");
            setRankType("saves");
          }}
          className={`px-3 py-2 text-xs font-black tracking-wide border-b-2 transition-all ${
            subTab === "goalkeeper_round"
              ? "border-amber-500 text-amber-400 font-extrabold"
              : "border-transparent text-slate-400 hover:text-slate-200"
          }`}
        >
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select
              ...
              className="bg-black/80 border border-gray-800 rounded-lg px-3 text-xs text-slate-200 h-9 cursor-pointer focus:outline-none focus:border-green-500"
            >
          ...
          <div className="flex flex-col gap-1.5 transition-all">
            {subTab === "round" || subTab === "goalkeeper_round" ? (
              <>
                <label className="text-[11px] font-bold text-green-400 uppercase tracking-wider">
                  🔢 选择具体轮次
                </label>
                <select
                  value={searchRound}
                  onChange={(e) => setSearchRound(e.target.value)}
                  className="bg-black/80 border border-green-900/60 text-green-400 rounded-lg px-3 text-xs h-9 cursor-pointer focus:outline-none focus:border-green-500 font-mono font-bold"
                >
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
          <h2 className="text-base font-black text-slate-900 tracking-wide">
            ...
            {(subTab === "round" || subTab === "goalkeeper_round") &&
              `第 ${searchRound} 轮`}
            <span className="font-extrabold ml-1 text-emerald-600">
              {subTab === "total" &&
                (rankType === "goals" ? "青年新星进球榜" : "青年新星助攻榜")}
              {subTab === "round" && "青年新星进球助攻榜"}
              {subTab === "goalkeeper_total" &&
                (rankType === "saves" ? "青年新星扑救榜" : "青年新星零封榜")}
              {subTab === "goalkeeper_round" && "青年新星扑救零封榜"}
            </span>
          </h2>
          ...
          {/* 🧤 门将赛季榜单药丸 */}
          {subTab === "goalkeeper_total" && (
            <div className="flex bg-amber-50 p-0.5 rounded-lg border border-amber-200 screenshot-hide-btn">
              <button
                onClick={() => setRankType("saves")}
                className={`px-3 py-1 text-[11px] font-black rounded-md transition-all ${rankType === "saves" ? "bg-white text-amber-900 shadow-sm" : "text-amber-500 hover:text-amber-700"}`}
              >
                🧤 扑救榜
              </button>
              <button
                onClick={() => setRankType("clean_sheets")}
                className={`px-3 py-1 text-[11px] font-black rounded-md transition-all ${rankType === "clean_sheets" ? "bg-white text-amber-900 shadow-sm" : "text-amber-500 hover:text-amber-700"}`}
              >
                🛡️ 零封榜
              </button>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 px-1 text-xs">
          <div className="text-slate-500 font-medium whitespace-nowrap flex items-center flex-wrap gap-1.5">
            ...
            <span className="font-mono font-black text-emerald-600 text-sm bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100">
              {currentPlayerCount}
            </span>
          ...
          <table className="min-w-[800px] w-full text-left border-collapse table-fixed">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs font-bold">
              <tr>
                ...
                {(subTab === "total" || subTab === "goalkeeper_total") && (
                  <th className="w-24 text-center">产出轮次</th>
                )}

                {/* 🎯 动态渲染表头：赛季榜只根据状态显示单列 */}
                {subTab === "total" &&
                  (rankType === "goals" ? (
                    <th className="w-20 text-center text-xs font-bold">
                      ⚽ 进球数
                    </th>
                  ) : (
                    <th className="w-20 text-center text-xs font-bold">
                ...
                {subTab === "goalkeeper_total" &&
                  (rankType === "saves" ? (
                    <th className="w-20 text-center text-xs font-bold text-amber-700">
                      🧤 扑救数
                    </th>
                  ) : (
                    <th className="w-20 text-center text-xs font-bold text-blue-700">
                      🛡️ 零封场次
                    </th>
                  ))}
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td
                    colSpan="8"
                    className="text-center py-16 text-slate-400 text-xs animate-pulse"
                  >
                    ⚡ 正在处理实时数据库...
                  </td>
                </tr>
                ...
                (subTab.startsWith("goalkeeper")
                  ...
                  const pName =
                    subTab === "total" || subTab === "goalkeeper_total"
                      ? item.name
                      : item.players?.name || "未知";
                  const pBirth =
                    subTab === "total" || subTab === "goalkeeper_total"
                      ? item.birth_year
                      : item.players?.birth_year || "--";
                  const cName =
                    subTab === "total" || subTab === "goalkeeper_total"
                      ? item.club_name
                      : item.clubs?.name || "自由身";
                  ...
                  return (
                    <tr
                      key={item.id}
                      className="text-xs text-slate-700 border-b border-slate-100 transition-colors"
                      style={{ backgroundColor: getRowBgColor(index, lName) }}
                    >
                      <td className="text-center py-3.5 font-mono font-black text-slate-400/80 text-[13px]">
                        {index + 1}
                      </td>
                      <td className="font-black text-slate-900 text-[14px] tracking-wider truncate">
                        {pName}
                      </td>
                      ...
                      <td className="pl-4 font-black text-emerald-800 truncate">
                        {cName}
                      </td>
                      {(subTab === "total" ||
                        subTab === "goalkeeper_total") && (
                      ...
                      {/* 🎯 动态渲染数据单元格：赛季榜单独对应单列展示 */}
                      {subTab === "total" &&
                        (rankType === "goals" ? (
                          <td className="text-center font-mono font-black text-[14px] text-green-700">
                            {item.goals}
                          </td>
                        ) : (
                      ...
                      {subTab === "goalkeeper_total" &&
                        (rankType === "saves" ? (
                          <td className="text-center font-mono font-black text-[14px] text-amber-700">
                            {item.saves}
                          </td>
                        ) : (
                          <td className="text-center font-mono font-black text-[14px] text-blue-700">
