...
export default function FixturesTab() {
  ...
  useEffect(() => {
    ...
    window.location.hash = queryString ? `#/fixtures?${queryString}` : '#/fixtures';
  ...
  const filteredFixtures = useMemo(() => {
    ...
    return fixtures.filter(item => {
      ...
      const matchTeam = teamName.toLowerCase().includes(searchTeam.toLowerCase());
      ...
      const matchMonthCond = searchMonth === 'all' || matchMonth === searchMonth;
      ...
      const matchResultCond = searchResult === 'all' || item.result === searchResult;

      return matchTeam && matchYearCond && matchMonthCond && matchStatusCond && matchResultCond;
    });
  }, [fixtures, searchTeam, searchYear, searchMonth, searchStatus, searchResult]);
  ...
  const dynamicTitleText = useMemo(() => {
    ...
    const monthPart = searchMonth !== 'all' ? `${parseInt(searchMonth)}月份 ` : '全年';
  ...
  const exportToImage = async () => {
    ...
    try {
      const dataUrl = await domToPng(shareAreaRef.current, { 
        quality: 1, 
        scale: 2, 
        backgroundColor: '#ffffff',
        filter: (node) => {
          if (node.classList && (
            node.classList.contains('screenshot-hide-btn') || 
            node.classList.contains('screenshot-hide-text')
          )) {
            return false;
          }
          return true;
      ...
      link.download = `CFNTA_国字号赛程战绩_${new Date().toISOString().slice(0,10)}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
...
// 📄 B. 工业级 PDF 单页高清导出机制 (精准改良：锁定真实函数名，绝不误伤包含‘胜平负’字样的比赛备注)
  ...
  return (
    <div className="space-y-5">
      ...
      {/* 控制检索工具栏 */}
      <div className="grid gap-3 grid-cols-2 md:grid-cols-6 bg-gray-900 bg-opacity-60 border border-gray-800 p-4 rounded-xl">
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold text-gray-400">🛡️ 队伍序列</label>
          <input placeholder="如: U19男足" value={searchTeam} onChange={(e) => setSearchTeam(e.target.value)} className="bg-black border border-gray-800 rounded-lg px-3 text-xs h-9 text-gray-200 focus:outline-none focus:border-green-500" />
        </div>

        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold text-gray-400">📅 按年度检索</label>
          <select value={searchYear} onChange={(e) => setSearchYear(e.target.value)} className="bg-black border border-gray-800 rounded-lg px-3 text-xs text-gray-200 h-9 cursor-pointer focus:outline-none focus:border-green-500">
            <option value="all">全部年份</option>
            {availableYears.map(year => (
              <option key={year} value={year}>{year}年 赛季</option>
            ))}
          </select>
        </div>
        
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold text-gray-400">📆 按指定月份</label>
          <select value={searchMonth} onChange={(e) => setSearchMonth(e.target.value)} className="bg-black border border-gray-800 rounded-lg px-3 text-xs text-gray-200 h-9 cursor-pointer focus:outline-none focus:border-green-500">
            <option value="all">全年所有月份</option>
            {['01','02','03','04','05','06','07','08','09','10','11','12'].map(m => (
              <option key={m} value={m}>{parseInt(m)}月份</option>
            ))}
          </select>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <select value={searchStatus} onChange={(e) => setSearchStatus(e.target.value)} className="bg-black border border-gray-800 rounded-lg px-3 text-xs text-gray-200 h-9 cursor-pointer focus:outline-none focus:border-green-500">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <select value={searchResult} onChange={(e) => setSearchResult(e.target.value)} className="bg-black border border-gray-800 rounded-lg px-3 text-xs text-gray-200 h-9 cursor-pointer focus:outline-none focus:border-green-500">
        ...
        <div className="flex items-end gap-1.5 justify-end">
          <button onClick={exportToImage} className="bg-green-600 hover:bg-green-500 text-white font-bold text-xs px-2.5 h-9 rounded-lg shadow-md focus:outline-none transition-colors shrink-0">📸 图片</button>
          <button onClick={exportToPDF} className="bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs px-2.5 h-9 rounded-lg shadow-md focus:outline-none transition-colors shrink-0">📄 PDF</button>
        </div>
      </div>

      {/* 📊 画布区 */}
      <div ref={shareAreaRef} className="p-5 rounded-xl border border-gray-200 space-y-4" style={{ backgroundColor: '#ffffff', color: '#0f172a' }}>
        ...
        {/* 战绩快照 */}
        <div className="grid grid-cols-4 gap-2 p-3 rounded-lg border border-gray-200 text-center" style={{ backgroundColor: '#f9fafb' }}>
          <div className="border-r border-gray-200">
            <div className="text-xs text-gray-500 font-medium">总完赛场次</div>
            <div className="text-base font-black text-gray-800 mt-0.5">{stats.total}</div>
          </div>
          <div className="border-r border-gray-200">
            <div className="text-xs text-green-600 font-medium">🟢 胜场</div>
            <div className="text-base font-black text-green-600 mt-0.5">{stats.win}</div>
          </div>
          <div className="border-r border-gray-200">
            <div className="text-xs text-amber-600 font-medium">🟠 平局</div>
            <div className="text-base font-black text-amber-600 mt-0.5">{stats.draw}</div>
          </div>
          <div>
            <div className="text-xs text-red-600 font-medium">🔴 负场</div>
            <div className="text-base font-black text-red-600 mt-0.5">{stats.loss}</div>
          </div>
        </div>
            ...
            <tbody>
              {filteredFixtures && filteredFixtures.length > 0 ? (
                filteredFixtures.map((fix) => {
                  ...
                  return (
                    <tr key={fix.id} className="border-b border-gray-200 text-xs transition-colors" style={{ backgroundColor: rowBgColor, color: rowTextColor }}>
                      <td className="pl-6 py-3 font-mono font-bold">{formatMatchDate(fix.match_date)}</td>
                      <td className="font-mono opacity-80">{fix.match_time ? fix.match_time.substring(0, 5) : '--'}</td>
                      
                      {/* 🟢 赛事名称单元格：配合 w-52，去除截断隐患 */}
                      <td className="pl-4 font-semibold truncate text-gray-700">{fix.tournament_name}</td>
                      
                      <td className="font-black truncate">{fix.national_teams?.name || '未知序列'}</td>
                      
                      <td className="text-center py-3">
                        <div className="flex items-center justify-center w-full">
                          <span className="rounded text-[11px] font-bold border shadow-sm flex items-center justify-center font-mono" style={{ width: '56px', height: '20px', backgroundColor: badgeBgColor, color: badgeTextColor, borderColor: 'transparent' }}>
                            {isFinished ? `${fix.our_score}-${fix.opponent_score}` : 'VS'}
                          </span>
                        </div>
                      </td>
                      
                      <td className="pl-4 font-black truncate">{fix.opponent}</td>
                      
                      <td className="pl-4 py-3 whitespace-nowrap overflow-hidden">
                        <div className="flex items-center gap-2 max-w-full overflow-hidden">
                          {fix.notes && (
                            <span className="text-[11px] bg-white text-gray-700 px-1.5 py-0.5 rounded border border-gray-200 shadow-sm inline-block truncate max-w-[220px]" title={fix.notes}>
                              ℹ️ {fix.notes}
                            </span>
                          )}
                ...
                <tr><td colSpan="7" className="text-center py-16 text-gray-400 text-xs">当前检索条件下，暂无对应的国字号赛事数据安排。</td></tr>
              )}
            </tbody>
          </table>
        </div>
