...
export default function SquadsTab() {
  ...
  const availableYears = useMemo(() => {
    if (!rawSquads) return [];
    return rawSquads
      .map(item => item.tournament_year ? item.tournament_year.toString() : null)
  ...
  const positionWeights = {
    '门将': 1, 'GK': 1, 'goalkeeper': 1,
    '后卫': 2, 'DF': 2, 'defender': 2, '边后卫': 2, '中后卫': 2,
    '中场': 3, 'MF': 3, 'midfielder': 3, '前腰': 3, '后腰': 3, '边锋': 3,
    '前锋': 4, 'FW': 4, 'forward': 4, '前锋/中锋': 4, '中锋': 4
  };
  ...
  const filteredSquads = useMemo(() => {
    ...
    const filtered = rawSquads.filter(item => {
      ...
      const matchTournament = tournamentName.toLowerCase().includes(searchTournament.toLowerCase());
      const matchYearCond = searchYear === 'all' || (item.tournament_year ? item.tournament_year.toString() : '') === searchYear;
  ...
  // 🟢 核心修改：生成图片并复制到剪切板（已彻底删掉所有会打断操作的 alert）
  const exportToImageAndCopy = async () => {
    ...
    try {
      ...
      link.download = `CFNTA_大赛名单球员现状_${new Date().toISOString().slice(0,10)}.png`;
      link.href = dataUrl;
      link.click();
      ...
      console.log('大名单图片已保存至本地并成功同步至剪切板。');
    } catch (err) {
      console.error('图片处理或复制剪切板失败:', err);
    }
  };
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 grid-cols-1 md:grid-cols-3 bg-gray-900 bg-opacity-60 border border-gray-800 p-4 rounded-xl">
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold text-gray-400">🏆 大赛名称检索</label>
          <input 
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            ...
            {availableYears.map(year => <option key={year} value={year}>{year}年 大赛名单</option>)}
          </select>
        </div>
        <div className="flex items-end justify-end">
          <button 
            onClick={exportToImageAndCopy} 
      ...
      >
        <div className="text-center py-2 border-b border-gray-200">
          <h2 className="text-base font-black text-gray-900 tracking-wide">{dynamicHeaderTitle}</h2>
        </div>
        ...
        <div className="bg-white border border-gray-300 rounded-lg overflow-hidden shadow-sm w-full">
          <table className="w-full text-left border-collapse table-fixed">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan="7" className="text-center py-16 text-gray-400 text-xs animate-pulse">⚡ 正在加载最新名单大盘...</td></tr>
                ...
                filteredSquads.map((player, index) => {
                  ...
                  if (posText.includes('后卫') || posText === 'DF') {
                    ...
                    posBg = '#e8f5e9';      
                    posColor = '#2e7d32';
                  } else if (posText.includes('前锋') || posText === '中锋' || posText === 'FW') {
                    posBg = '#fff3e0';      
                    posColor = '#e65100';
                  }

                  return (
                    <tr key={player.id} className="text-xs text-gray-800 border-b border-gray-200 transition-colors" style={{ backgroundColor: rowBgColor }}>
                      <td className="pl-6 py-3.5 font-black text-gray-900 truncate text-[14px] tracking-wider">{player.player_name}</td>
                      <td className="text-center font-mono font-bold text-gray-700">{player.birth_year || '--'}</td>
                      ...
                      <td className="pl-4 truncate font-bold text-gray-600">
                        {player.current_league && player.current_league.trim() !== '' ? player.current_league : ''}
                      </td>
                      ...
                      <td className="pl-4 py-2 text-left">
                        <div className="flex flex-col gap-1 max-w-full">
                          <span className="font-black text-emerald-700 text-[13px]">
                            {player.current_club && player.current_club.trim() !== '' ? player.current_club : ''}
                          </span>
                      ...
                      {/* 现状更新日期 */}
                      <td className="text-center font-mono text-gray-400 text-[11px]">{formatDate(player.update_date)}</td>
                    </tr>
                  );
                })
              ) : (
                // 优雅阻断占位符
                <tr>
                  <td colSpan="7" className="text-center py-20 text-gray-400 text-xs tracking-wide">
